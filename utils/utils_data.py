...
def load_data(filename, rg):
    ...
    #df['E_ads'] = df['E_ads'].astype(float)
    df['E_ads'] = df['E_ads'].apply(eval).apply(np.array)
...
def plot_example(df, i=12, label_edges=False):
    ...
    #                 edge_color='gray')
    nx.draw_networkx(g, ax=ax[1], labels=node_labels, pos=pos, node_size=500, node_color=color, edge_color='gray')
